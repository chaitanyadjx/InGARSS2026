...
export default function DeadlineExtensionPopup() {
    ...
    const [isCCSVisible, setIsCCSVisible] = useState(false);

    useEffect(() => {
        // Check if user has already dismissed the July 15 popup
        const paperDismissed = localStorage.getItem('paperDeadlineJuly15Dismissed');
        if (!paperDismissed) {
            setTimeout(() => setIsPaperVisible(true), 1500);
        }

        // Check if user has already dismissed the CCS popup
        const ccsDismissed = localStorage.getItem('ccsSubmissionDismissed');
        if (!ccsDismissed) {
            setTimeout(() => setIsCCSVisible(true), 2000);
        }
    }, []);

    const handlePaperDismiss = () => {
        setIsPaperVisible(false);
        localStorage.setItem('paperDeadlineJuly15Dismissed', 'true');
    };

    const handleCCSDismiss = () => {
        setIsCCSVisible(false);
        localStorage.setItem('ccsSubmissionDismissed', 'true');
    };

    if (!isPaperVisible && !isCCSVisible) return null;

    return (
        <div className="fixed bottom-4 right-4 md:bottom-8 md:right-8 z-50 max-w-sm w-[calc(100%-2rem)] flex flex-col gap-4">
            
            {/* PAPER SUBMISSION POPUP */}
            {isPaperVisible && (
                <div className="bg-white border-[3px] border-black shadow-[8px_8px_0_black] p-5 relative transition-transform duration-300">
                    <button
                        onClick={handlePaperDismiss}
                    ...
                    <div className="flex gap-4 items-start pr-4">
                        ...
                        <div>
                            <h3 className="font-extrabold text-lg leading-tight mb-1 text-black">Paper Submission Extended!</h3>
                            <p className="text-sm opacity-80 mb-3 text-black">
                                Submit your papers before the hard deadline on <strong>15th July 2026</strong>.
                            </p>
